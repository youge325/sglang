...
本节描述 Ascend NPU 支持的基本功能和特性。如果您遇到问题或有任何疑问，请[提交 Issue](https://github.com/sgl-project/sglang/issues)。

如需了解各参数的含义和用法，请点击 [服务器参数](https://docs.sglang.io/advanced_features/server_arguments.html)。
...
| `--quantize-and-serve`                      | `False`  | bool flag <br/> (set to enable)         | GPU 专用          |
...
## 内核后端（Attention、采样、Grammar、GEMM）
...
| `--fp8-gemm-backend`                   | `auto`            | `auto`,<br/> `deep_gemm`,<br/> `flashinfer_trtllm`,<br/>`cutlass`,<br/> `triton`,<br/> `aiter` | GPU 专用          |
| `--disable-flashinfer-`<br/>`autotune` | `False`           | bool flag<br/> (set to enable)                                                                 | GPU 专用          |
...
| `--speculative-ngram-`<br/>`match-type`            | `BFS`      | `BFS`,<br/> `PROB` |   实验性        |
| `--speculative-ngram-`<br/>`branch-length`         | `18`       | Type: int          |   实验性        |
...
| `--enable-flashinfer-`<br/>`allreduce-fusion`         | `False`   | bool flag<br/> (set to enable)              | GPU 专用          |
...
| 参数                | 默认值   | 选项                            | 服务端支持         |
|--------------------|----------|--------------------------------|:----------------:|
| `--enable-lmcache` | `False`  | bool flag<br/> (set to enable) | GPU 专用          |
...
| `--enable-nccl-nvls`                                    | `False`  | bool flag<br/> (set to enable) | GPU 专用          |
| `--enable-symm-mem`                                     | `False`  | bool flag<br/> (set to enable) | GPU 专用          |
| `--disable-flashinfer-`<br/>`cutlass-moe-fp4-allgather` | `False`  | bool flag<br/> (set to enable) | GPU 专用          |
...
| `--enable-mscclpp`                                      | `False`  | bool flag<br/> (set to enable) | GPU 专用          |
| `--enable-torch-`<br/>`symm-mem`                        | `False`  | bool flag<br/> (set to enable) | GPU 专用          |
...
| `--enable-two-`<br/>`batch-overlap`                     | `False`  | bool flag<br/> (set to enable) |     计划中        |
...
| `--enable-p2p-check`                                    | `False`  | bool flag<br/> (set to enable) | GPU 专用          |
| `--triton-attention-`<br/>`reduce-in-fp32`              | `False`  | bool flag<br/> (set to enable) | GPU 专用          |
...
| `--flashinfer-mla-`<br/>`disable-ragged`                | `False`  | bool flag<br/> (set to enable) | GPU 专用          |
...
| `--enable-layerwise-`<br/>`nvtx-marker`                 | `False`  | bool flag<br/> (set to enable) | GPU 专用          |
| `--enable-attn-tp-`<br/>`input-scattered`               | `False`  | bool flag<br/> (set to enable) |   实验性          |
| `--enable-nsa-prefill-`<br/>`context-parallel`          | `False`  | bool flag<br/> (set to enable) |      A2, A3      |
| `--enable-fused-qk-`<br/>`norm-rope`                    | `False`  | bool flag<br/> (set to enable) | GPU 专用          |
...
| `--remote-instance-weight-`<br/>`loader-start-seed-via-transfer-engine` | `False`  | bool flag<br/> (set to enable)  | GPU 专用          |

## PD-Multiplexing

| 参数                   | 默认值   | 选项                            | 服务端支持         |
|-----------------------|----------|--------------------------------|:----------------:|
| `--enable-pdmux`      | `False`  | bool flag<br/> (set to enable) | GPU 专用          |
...
| 参数                                       | 默认值   | 选项                            | 服务端支持         |
|-------------------------------------------|----------|--------------------------------|:----------------:|
| `--enable-deterministic-`<br/>`inference` | `False`  | bool flag<br/> (set to enable) |     计划中        |
...
以下参数不受支持，因为其依赖的第三方组件与 NPU 不兼容，如 Ktransformer、checkpoint-engine 等。
