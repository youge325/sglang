...
#### 在 1 台 Atlas 800I A3 上以 PD 混合模式运行 DeepSeek

W4A8 模型权重可在[此处](https://modelers.cn/models/Modelers_Park/DeepSeek-R1-0528-w4a8)找到。
...
#### 在 2 台 Atlas 800I A3 上以 PD 分离模式运行 DeepSeek
...
# 建议 max-running-requests <= max-cuda-graph-bs * dp_size，因为超过此值时性能会显著下降。
python -m sglang.launch_server \
    --model-path ${MODEL_PATH} \
...
3. SGLang Model Gateway（前身为 Router）
...
#### 在 4 台 Atlas 800I A3 上以 PD 分离模式运行 DeepSeek

W8A8 模型权重可在[此处](https://modelers.cn/models/State_Cloud/Deepseek-R1-bf16-hfd-w8a8)找到。
...
2. SGLang Model Gateway（前身为 Router）：
