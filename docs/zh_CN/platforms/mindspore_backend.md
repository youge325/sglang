...
MindSpore 目前仅支持 Ascend NPU 设备。用户需要先安装 Ascend CANN 软件包。
...
> **注意**：目前，MindSpore 模型由独立包 `sgl-mindspore` 提供。MindSpore 的支持建立在当前 SGLang 对 Ascend NPU 平台支持的基础上。请先[安装适用于 Ascend NPU 的 SGLang](ascend_npu.md)，然后安装 `sgl-mindspore`：
...
llm = sgl.Engine(
    ...
    attention_backend="ascend",        # Attention 后端
    tp_size=1,                         # 张量并行大小
    dp_size=1                          # 数据并行大小
)
...
在某些特殊 protobuf 版本的环境下，用户需要设置环境变量以避免二进制版本不匹配的问题。
